...
def goal_change(gx,gy,gz,oz,ow):
    global old_gx, old_gy, old_gz, old_oz, old_ow, coffee_mode
    ...
    if test_flag == False:
        ...
        if coffee_mode:
            ...
            play_pub.publish(play2)
            time.sleep(1)  # 실행할 명령어를 전송하기 위해 잠시 대기
            audio_file = "{}/audio/coffee_finish.mp3".format(image_path)  # 재생할 오디오 파일 경로
            playsound(audio_file)
            msg_box = QMessageBox.question(map_widget, "확인창", "목적지에 도착하였습니다. 트레이에서 음료를 가져가세요. 초기 위치로 복귀할까요?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        else:
            play2 = Int32()
            play2.data = 2
            play_pub.publish(play2)
...
if __name__ == '__main__':
    global test_flag, status, coffee_mode
    coffee_mode = False
